...
## Code Style
- **Backend**: Follow FastAPI architecture rules in `.qwen/instructions/backend-fastapi-architecture.md`
- **Frontend**: Follow React TypeScript rules in `.qwen/instructions/frontend-react-typescript.md`
...
- **Backend tests**: `pytest` in backend/ directory, minimum 80% coverage enforced. See `.qwen/instructions/testing-coverage-gate.md`
- **Frontend**: `npm run dev` for development, `npm run build` for production.
- **AWS deployment**: use the CDK app in `infra/cdk` and follow `.qwen/instructions/aws-cdk-deployment.md`.
